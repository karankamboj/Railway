...
class Karan:
    ...
    def userpage(self):
        ...
        sourcelist=list(c.execute("Select distinct Source from trains")) #fetching source from database
        for i in range(0,len(sourcelist)):
            sourcelist[i]=str(sourcelist[i])
            sourcelist[i]=sourcelist[i].replace('''',)''',"")
            sourcelist[i]=sourcelist[i].replace("('","")

        destinationlist=list(c.execute("Select distinct Source from trains")) #feting destination from database
        for i in range(0,len(destinationlist)):
            destinationlist[i]=str(destinationlist[i])
            destinationlist[i]=destinationlist[i].replace('''',)''',"")
            destinationlist[i]=destinationlist[i].replace("('","")

        self.f1=OptionMenu(self.master,self.variable1,*sourcelist)
        self.f1.place(x=380,y=150)
        self.variable2= StringVar(self.master)
        self.variable2.set("Select")
        self.f2=OptionMenu(self.master,self.variable2,*destinationlist)
